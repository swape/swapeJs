...
var sjs = function () {
					...
					for (var x = 0; x < elm.length; x++) {
						...
						if (elm[x]) {
							...
							if (elm[x][attr]) {
								...
								if (data) {
									elm[x][attr] = elm[x][attr].replace(data, '').trim();
								} else {
	...
	function runFromRoute(inValue) {
		...
		if (inValue.template) {
			...
			if (templateCache[inValue.templateUrl]) {
				...
				getXhr('get', inValue.templateUrl).then(function (data) {
					view.innerHTML = data;
					runController(inValue);
	...
	function attachTemplate(obj, url) {
		return new Promise(function (resolve, reject) {
			...
			if (templateCache[url]) {
				...
				// getting the external template from url
				getXhr('get', url).then(function (data) {
					obj.innerHTML = data;
	...
	function getHash(inValue) {
		...
		if (objRoutes.hash !== '') {
			objRoutes.hash = objRoutes.hash.replace(objRoutes.startHashSign, '').replace('#', '');
